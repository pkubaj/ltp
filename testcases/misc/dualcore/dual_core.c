...
static int *list_core_type; // 0- P core, 1 - E core, 2 - LPE
static uint64_t *array_usage;
static bool *list_initial_cpu_state;

static bool has_core_l3_cache(int i)
{
	char path[PATH_MAX];
...
static void count_cores(void)
{
	...
	buf = SAFE_MALLOC(PATH_MAX * sizeof(char));

	SAFE_FILE_SCANF("/sys/devices/cpu_core/cpus", "%s", buf);
	buf[strcspn(buf, "\n")] = '\0';
	...
	while (*p) {
		...
		if (sscanf(p, "%d-%d%n", &start, &end, &n) == 2) {
			if (start > end) {
				tst_brk(TBROK, "Invalid CPU range %d-%d", start, end);
			}
	...
	SAFE_FSEEK(p, 0, SEEK_SET);

	SAFE_FILE_SCANF("/sys/devices/cpu_atom/cpus", "%s", buf);
	buf[strcspn(buf, "\n")] = '\0';
	p = buf;
	...
	while (*p) {
		...
		if (sscanf(p, "%d-%d%n", &start, &end, &n) == 2) {
			...
			if (has_core_l3_cache(start)) {
				list_core_type[start] = 1;
				number_e_core++;
			} else {
				list_core_type[start] = 2;
				number_lpe_core++;
			}
		}
		p += n;
...
static void count_utilized_cores(uint64_t *usage_per_core_array, const int expected_p, const int expected_e, const int expected_lpe)
{
	...
	if (p_usage == expected_p && e_usage == expected_e && lpe_usage == expected_lpe)
		tst_res(TPASS,
...
static uint64_t *count_usage_per_cpu(void)
{
	...
	for (int i = 0; i < nproc; i++) {
		...
		tst_res(TINFO, "CPU%d Usage: %.2f%%", i, cpu_usage);
