...
static void check_module_initialization(const char *module, char *marker)
{
	...
	if (params.found)
		...
		tst_res(TWARN, "Unproperly configuration. Missing cmdline parameter: '%s'.", kcmdline_param);
	};
...
{
	const char *mgs = "Not loads module. Module %s is built in driver.";
...
{
	const char *test_corrected = "testcases/data/mce-inject/test/corrected";
